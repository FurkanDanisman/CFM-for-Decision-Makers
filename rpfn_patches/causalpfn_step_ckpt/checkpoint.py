"""Modified CausalPFN Checkpoint callback.

CHANGE vs upstream: accept `actual_step` kwarg from the trainer and store
it in the per-epoch saved dict under the key `"actual_step"`. This lets
resume paths carry the true optimizer-step counter (see modified
trainer.py which increments only after a successful optimizer.step()).

Backwards compatible: the kwarg defaults to None; old callers (that do
not pass it) still work exactly as before, and the key is simply omitted
from the save dict.
"""
import os
...
class Checkpoint(Callback):
    ...
    def __call__(
        ...
        wandb_enabled: bool,
        lr_scheduler=None,
        actual_step: int | None = None,  # ── STEP-CKPT ── new optional kwarg
    ):
        if (epoch + 1) % self.frequency == 0:
            ...
            # ── STEP-CKPT ── carry true optimizer-step counter into per-epoch save
            if actual_step is not None:
                training_state["actual_step"] = int(actual_step)
