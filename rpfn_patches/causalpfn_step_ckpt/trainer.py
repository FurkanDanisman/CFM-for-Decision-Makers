...
def _save_step_checkpoint(
    ...
    training_state = {
        ...
        # top_scores/epochs left empty on step ckpts — per-epoch Checkpoint
        # callback owns top-k tracking.
        "top_scores": [],
        "top_epochs": [],
    }
...
def train(
    ...
    if checkpoint:  # load optimizer state if resume = True
        ...
        # ── STEP-CKPT PATCH ── restore true optimizer-step counter (0 for legacy ckpts)
        actual_step = int(checkpoint.get("actual_step", 0))
    else:
        start_epoch = 0
        actual_step = 0
    ...
    # ── STEP-CKPT PATCH ── resolve output dir + run-name for step-based saves.
    # Reuses the same run-subdir as the per-epoch Checkpoint callback (if any).
    step_ckpt_dir = None
    _run_name = None
    _dir_name = None
    for cb in callbacks:
        if hasattr(cb, "default_name") and hasattr(cb, "checkpoint_dir_name"):
            _run_name = cb.default_name
            _dir_name = cb.checkpoint_dir_name
            break
    if step_checkpoint_every and step_checkpoint_every > 0:
        assert step_checkpoint_root, "step_checkpoint_root is required when step_checkpoint_every > 0"
        from datetime import datetime
        run_subdir = _dir_name if _dir_name is not None else datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        step_ckpt_dir = os.path.join(step_checkpoint_root, run_subdir)
        if rank == 0:
    ...
    for epoch in range(max_epochs):
        ...
        for batch_counter in range(num_agg * num_model_updates):
            ...
            if last_micro_batch:
                ...
                # ── STEP-CKPT PATCH ── advance true optimizer-step counter (only
                # after a successful, non-skipped opt.step()) and periodically
                # save a step-tagged checkpoint.
                actual_step += 1
                if (
                    step_checkpoint_every
                    and step_checkpoint_every > 0
        ...
        for i, callback in enumerate(callbacks):
            ...
            # ── STEP-CKPT PATCH ── pass actual_step ONLY to callbacks whose
            # signature accepts it (Checkpoint accepts it after our patch;
            # every other callback keeps its original invocation).
            _base_kwargs = dict(
                callback_idx=i,
            ...
            try:
                _sig = inspect.signature(callback.__call__)
                _accepts = ("actual_step" in _sig.parameters) or any(
                    p.kind == inspect.Parameter.VAR_KEYWORD for p in _sig.parameters.values()
                )
            except (TypeError, ValueError):
                _accepts = False
            if _accepts:
                _base_kwargs["actual_step"] = actual_step
            callback(**_base_kwargs)
