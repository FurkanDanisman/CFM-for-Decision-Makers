...
def main():
    ...
    ap.add_argument("--rho99-root", default=None,
                    help="data root of the rho-filtered ComplexMech cells "
                         "(default $SCRATCH/cmech_data_rho99)")
    ...
    # ComplexMech restricted to strongly-coupled realizations. Reported as its own row
    # because it is a DIFFERENT population, not a subset summary: those cells were
    # generated by rejecting draws with rho <= the threshold, so the row answers "what
    # is rho once the benchmark's target mismatch is removed" rather than describing
    # the main benchmark. The threshold is named in the row so the selection is not
    # mistaken for a property of the prior.
    for root, thr in ((a.rho99_root or os.path.join(SC, "cmech_data_rho99"), "0.99"),):
        cm99 = sorted(glob.glob(os.path.join(root, "complexmech",
                                             "*node", "*", "*", "r*.npz")))
        if cm99:
            r = _npz_rows(f"ComplexMech (rho > {thr})", cm99, "Y_do0", "Y_do1",
                          "X_test", "r*.npz realization", cap)
            if r:
                rows.append(r)
        else:
            print(f"note: no ComplexMech rho-filtered npz under {root}",
                  file=sys.stderr)
