...
echo "throttle: $THROTTLE concurrent tasks per array"
echo
    ...
    if [ "$SUBMIT" = 1 ]; then
        gid=$(CASE="$case" SHIFT="$sh" DS="$DS" REALS="$REALS" QUERIES="$QUERIES" \
              ...
              sbatch --parsable --account="$ACCT" \
                     ...
                     "$REPO/benchmarks/cluster/submit_fq4_gen.sbatch")
        # afterok on the whole gen array: a dump that starts against a half-written
        # cell reads a truncated npz and reports a number rather than failing.
        did=$(CASE="$case" SHIFT="$sh" DS="$DS" REALS="$REALS" QUERIES="$QUERIES" \
              CTX="$CTX" \
              sbatch --parsable --account="$ACCT" \
                     --array="0-$((DUMP_N-1))%$THROTTLE" \
                     --dependency="afterok:$gid" \
                     --job-name="fq4d-$case-$sh" \
                     "$REPO/benchmarks/cluster/submit_fq4_dump.sbatch")
        printf '%-36s shift%-3s gen=%s dump=%s\n' "$case" "$sh" "$gid" "$did"
    else
