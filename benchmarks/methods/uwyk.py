...
def build_no_ancestral_adjacency(model_n_features, n_real_features):
    """Zero adjacency: no DAG hint at inference. Padded feats still -1.

    Same input format as full-graph adjacency, but all "real" entries are 0 —
    the model gets no information about which nodes cause which. This is the
    standard proxy for "No Ancestral Info" when the paper's separately-trained
    No-Ancestral checkpoint isn't available.
    """
    adj = np.zeros((model_n_features + 2, model_n_features + 2), dtype=np.float32)
    for i in range(n_real_features, model_n_features):
        fi = 2 + i
...
def _uwyk_predict_cate(uwyk_model, cate_dataset, adjacency_builder):
    ...
    adjacency_matrix = adjacency_builder(model_n_features, n_real_features)
...
def uwyk_no_ancestral_pipeline(uwyk_model, cate_dataset):
    """UWYK, same checkpoint as Ancestral, but no DAG hint at inference.
    Standard proxy for the paper's 'No Ancestral Info' row."""
    return _uwyk_predict_cate(uwyk_model, cate_dataset, build_no_ancestral_adjacency)
