"""Per-realization density L2 evaluation on ACIC 2016.

Same protocol as l2_ihdp/eval_realization.py but ACIC-specific truth (Gaussian
approximation with sigma estimated from training residuals) and dataset load
(ACIC2016Dataset from CausalPFN).

Reuses methods_densities.py + all `_run_*` helpers from l2_ihdp/eval_realization
via direct import — no duplication.

Env vars:
  REPO, CAUSALPFN, DOPFN, UWYK_SRC, UWYK_CKPT_DIR (paths, same as l2_ihdp)
  CHECKPOINT50, CHECKPOINT_DOPFN_BB (Ours model checkpoints)
  METHODS   (comma list; supported: ours_fn50, uwyk_noanc, uwyk_anc, dopfn, ours_dopfn_bb)
  OUT       (output prefix, one shard per realization)
  REALIZATION (0..9 for ACIC)
  MALC_B, MALC_MAX_K, N_EVAL, UWYK_N_SAMPLES, N_CONTEXT (same defaults as l2_ihdp)
  ACIC_CACHE_DIR (optional local cache dir with x.csv and zymu_{1..10}.csv)
"""
...
def main() -> int:
    ...
    ap.add_argument('--out', default=os.environ.get('OUT', 'l2_acic/out'))
    ap.add_argument('--n-context', type=int,
                     default=int(os.environ.get('N_CONTEXT', 0)))
    ...
    ap.add_argument('--methods', default=os.environ.get(
        'METHODS', 'ours_fn50,uwyk_noanc,uwyk_anc,dopfn,ours_dopfn_bb'))
    ...
    truth = load_acic_truth(
        args.realization, y_train_full,
