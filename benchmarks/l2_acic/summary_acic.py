"""Two-block ACIC summary — same layout as summary_ihdp.py.

Block 1: sqrt(PEHE) + eps_ATE per method (with (MALC-mean) / (raw-mean) rows for
Ours methods).
Block 2: L2 density metrics for {Do-PFN, uwyk_noanc, uwyk_anc, ours_fn50,
ours_dopfn_bb} when those shards exist.
"""
...
def main() -> int:
    ...
    ap.add_argument('--shards-glob', required=True,
                    help='e.g. .../l2_acic/out_all_B500.r*.npz')
    args = ap.parse_args()
    ...
    METHODS = [
        ('Do-PFN',                     'dopfn'),
        ('UWYK-NoAnc',                 'uwyk_noanc'),
        ('UWYK-FullAnc',               'uwyk_anc'),
        ('Ours(fn=50)',                'ours_fn50'),
        ('Ours(DoPFN-bb 200K)',        'ours_dopfn_bb'),
    ]

    print()
    print('── Table 3 addition — ACIC ────────────────────────────────────────')
