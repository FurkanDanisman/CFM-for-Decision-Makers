"""ACIC-only diagnostic: compute 4 CATE estimators per realization × 2 anc modes.

For each ACIC realization × (anc=v3a, noanc):
  - raw:      E[Y] = Σ centres · p_marg     (current default)
  - em:       fixed-point Gaussian correction on p_marg  (current default)
...
Saves per-realization npz with all 8 (pehe, err) pairs. Reports summary.
...
Env vars:
    ...
    MALC_B            default 1000 — B_fit=B_select for fit_malc_inner
    MALC_MAX_K        default 3
    MALC_N_EVAL       default 101 — grid points for dmalc_2d
...
# ── Anc matrix builders (verbatim from eval_graph2d_realcause.py) ────────
...
# ── EM 1D mean (from existing eval, for parity) ─────────────────────────
...
def forward_pmat(model, X_train, T_train, Y_train_scaled, X_test, adj, J):
    """Single forward → p_mat of shape (N_q, J, J)."""
    ...
    return p_mat.astype(np.float64)   # (N_q, J, J)


def _em_tau_from_fit(fit):
    """pi-weighted per-component EM τ from 2D MALC fit."""
    if fit is None:
        return float('nan')
    mus = [c.mu_hat for c in fit.fits if c is not None]
    if not mus:
        return float('nan')
    mus = np.stack(mus)
    ...
    if s <= 0:
        return float('nan')
    weights = weights / s
    return float((weights * (mus[:, 1] - mus[:, 0])).sum())


def cate_estimators(p_mat, J, edges_scaled):
    """Given p_mat (N_q, J, J), return 4 CATE arrays each length N_q (on scaled y)."""
    N_q = p_mat.shape[0]
    centres = 0.5 * (edges_scaled[:-1] + edges_scaled[1:])

    # ── Raw & EM (marginal-based) ────────────────────────────────────────
    p_y0_all = p_mat.sum(axis=2)   # (N_q, J)  marginal over Y1 axis
    p_y1_all = p_mat.sum(axis=1)   # (N_q, J)
    ...
    # ── 2D MALC → diagonal-integrated p(τ) → E[τ] ─────────────────────────
...
def evaluate_realization(r, ds, model, J, F):
    ...
    for mode, adj in (('anc', build_anc_full(F, n_real)),
        ...
        p_mat = forward_pmat(model, X_tr, T_tr, Y_obs, X_te, adj, J)  # (N_q, J, J)
...
def main():
    ...
    print(f'[bootstrap] ckpt={CKPT}  out={OUT}  MALC_B={MALC_B}', flush=True)
    model, cfg = load_model(CKPT)
    J = cfg['J']; F = cfg['num_features']
    ds = ACIC2016Dataset()
    print(f'[bootstrap] ACIC n_tables={ds.n_tables}  J={J}  F={F}', flush=True)
    rows = []
    t0 = time.time()
    for r in range(ds.n_tables):
        ...
        for est in ('raw', 'em', 'malc1d', 'malc2d'):
            for mode in ('anc', 'noanc'):
                parts.append(f'{est[:6]}-{mode[:5]}={row[f"pehe_{est}_{mode}"]:6.3f}')
        print(f'r={r:02d}  ' + '  '.join(parts) + f'  ({time.time()-t0:.0f}s)', flush=True)
    ...
    print(f'\n══ ACIC summary  (n={len(rows)}, MALC_B={MALC_B}) ══')
    print(f'{"est":<8} {"mode":<6} {"pehe":>9} {"err":>7} {"n":>3}')
    for est in ('raw', 'em', 'malc1d', 'malc2d'):
        for mode in ('anc', 'noanc'):
            m_pehe, se_pehe, n = ms(f'pehe_{est}_{mode}')
            m_err,  se_err,  _ = ms(f'err_{est}_{mode}')
            print(f'{est:<8} {mode:<6} {m_pehe:>9.3f} {m_err:>7.3f} {n:>3d}')
