"""Train UWYK's graph-conditioned model with our 2D joint BarDistribution head.

Identical to UWYK's `full_conditioned` training path (graph_encoder + soft
attention on the adjacency matrix, partial-graph format with random edge
hiding) except:

  - Output head: our BarDistribution2D (output_dim = J**2 + 9 + 4)
  - Loss:       neg_log_prob_2d on joint (Y_do0, Y_do1) targets
  - Query has no T_intv (learned null token fills the slot)

Config is env-var-driven (see the CONFIG section) so we don't need a yaml
runtime dependency. Defaults are the 20k-steps / 20-h wall configuration.

Reuses ``PairedInterventionalDataset`` because it already emits ``anc_matrix``
in {-1, 0, +1} form with the same random-hide protocol UWYK's graph model
was trained on.
"""
import faulthandler
faulthandler.enable()
...
# Training — 20K steps for the 20h wall budget. Effective batch same as UWYK.
N_STEPS         = int(os.environ.get('N_STEPS', 20000))
...
# Checkpoints
CHECKPOINT_DIR    = os.environ.get('CHECKPOINT_DIR', './checkpoints_graph2d')
...
def subsample_task(batch, n_context, n_query):
    """Subsample N and M axes. Passes anc_matrix through unchanged
    (it's a per-task (F+2, F+2) matrix, not per-sample)."""
...
def main():
    ...
    for step in range(start_step + 1, N_STEPS + 1):
        ...
        if interrupted['flag']:
            ...
            print(f"  Exiting cleanly. Resubmit sbatch to resume.")
            sys.exit(0)
