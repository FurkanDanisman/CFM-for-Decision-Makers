...
CASE_STUDY COPY of benchmarks/eval_causalpfn2d/eval_causalpfn_v0_realcause.py.
The ONLY change is the DENSITY_DUMP block, which now saves per-arm (shift,
scale) so STD_MODE=per_arm can be dumped -- upstream saves arm0 only and
asserts pooled. Everything else, including all point-estimate logic, is
verbatim; `diff` against the original should show exactly one hunk.
"""
...
parser.add_argument('--dataset', type=str,
                    ...
                    choices=('IHDP', 'ACIC', 'CPS', 'PSID', 'PSID_bal') + _SCM_CASES + _CMECH_CASES)
...
def cate_raw_and_em(model, X_train, T_train, Y_train_raw, X_test,
    ...
    if os.environ.get('DENSITY_DUMP', '0') == '1':
        # ── case_study copy: PER-ARM AWARE DUMP ──────────────────────────────
        # Upstream saves only arm0's (shift, scale) and hard-asserts pooled,
        # with the message "extend the density dump to save per-arm stats".
        # This is that extension: both arms' stats are saved, so tau can be
        # reconstructed in RAW units even when the arms sit on different axes.
        # See case_study/density_eval/density_cpfn.py::cpfn1d_tau_density_raw.
        #
        # Log families are still refused: _un applies expm1, so the map from
        # scaled bins to raw outcome is NOT affine and the saved histogram
        # cannot be pushed forward by a (shift, scale) pair.
        if STD_MODE not in ('pooled', 'per_arm'):
            raise AssertionError(
                f'[DENSITY_DUMP] STD_MODE={STD_MODE!r} is a log family; the '
                'scaled->raw map is not affine, so a per-arm bin dump cannot '
                'be un-scaled. Use pooled or per_arm.')
        (m0, s0), (m1, s1) = arm0, arm1
        # Same consistency gate as upstream, but written for per-arm: rebuild
        # cate_raw from exactly the quantities being saved. Under pooled this
        # reduces to the upstream check because m0 == m1 cancels.
        cate_from_density = (e_y1_raw * s1 + m1) - (e_y0_raw * s0 + m0)
        _max_diff = float(np.max(np.abs(cate_from_density - cate_raw)))
        if not np.allclose(cate_from_density, cate_raw, atol=1e-4, rtol=1e-4):
            raise AssertionError(
                f'[DENSITY_DUMP] density mean disagrees with cate_raw '
                f'(max |d| = {_max_diff:.4g}) under STD_MODE={STD_MODE!r}. '
                f'The p0/p1 about to be saved are not the ones cate_raw came '
                f'from; refusing to write a density inconsistent with the '
                f'reported point PEHE.')
        dens = dict(
            edges=bin_edges_np.astype(np.float32),
            p_y0_scaled=p0.astype(np.float32),
            p_y1_scaled=p1.astype(np.float32),
            arm0_shift=np.float32(m0), arm0_scale=np.float32(s0),
            arm1_shift=np.float32(m1), arm1_scale=np.float32(s1),
            std_mode=np.str_(STD_MODE),
            # kept for readers that expect the pooled schema; only meaningful
            # when the two arms share scaling
            y_shift=np.float32(m0), y_scale=np.float32(s0),
        )
