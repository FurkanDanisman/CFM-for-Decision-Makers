"""cpfn1d case-study eval — sweeps (case study × N) through the SAME underlying
script Table1/cpfn1d.py uses (benchmarks/eval_causalpfn2d/eval_causalpfn_v0_realcause.py),
with our npz loader shadowed in via PYTHONPATH. See _common.py.

Usage:
    python case_study/eval/cpfn1d.py \\
        --data-root case_study/data_shift+2 --context-sizes 200 500 1000 \\
        --outdir /scratch/.../cs_eval/cpfn1d \\
        --ckpt /scratch/.../causalpfn_j1024_headrand_.../step_0050000.pt \\
...
def _args():
    ...
    p.add_argument("--ckpt", required=True, help="CausalPFN 1D checkpoint (.pt).")
...
def main():
    ...
    for case, n in C.cells(a.cases, a.context_sizes):
        ...
        env = C.cell_env(a.data_root, n, {
            ...
            "EVAL_MAX_CONTEXT": "",           # full context (= all N rows)
            "EVAL_CONTEXT_SEED": "1", "STD_MODE": a.std_mode, "COMPILE": a.compile,
            "SKIP_EM": a.skip_em, "DENSITY_DUMP": a.density_dump,
        })
        print(f"[cpfn1d] {case} N={n} std_mode={a.std_mode} "
              f"ckpt={os.path.basename(a.ckpt)}", flush=True)
        C.launch(["python", "-u", script, "--dataset", case], env,
                 dry_run=a.dry_run, label=f"cpfn1d {case} N={n}")
