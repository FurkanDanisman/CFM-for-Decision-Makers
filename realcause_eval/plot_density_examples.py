...
For each subplot: analytic truth Gaussian overlaid on the method's estimated
density (both on the same tight τ grid, raw Y units). Vertical dashed line
at each query's true CATE.

Only IHDP and ACIC supported (analytic Gaussian truth). Lalonde would need
the KDE truth pipeline (which requires the ITE-match plumbing already in
eval_density_metrics.py — extend if needed).
...
def main():
    ...
    print(f'[bootstrap] dataset={args.dataset}  realization={r}  queries={qi}',
          flush=True)
