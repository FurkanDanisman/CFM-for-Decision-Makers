...
Because $Q^{\star}$ is defined explicitly by
\eqref{eq:agueh-carlier}, no iterative optimisation is needed. Our
implementation evaluates each $Q_i$ on a common grid of probability
levels $\tau_1, \dots, \tau_M \in (0, 1)$, averages the quantile
values, and recovers the barycenter density on the original $\tau$-grid
by numerically inverting $Q^{\star}$ and differentiating the resulting
CDF. All steps are $\mathcal{O}(N\!M)$ vector arithmetic; the barycenter
is thus much cheaper than the per-query MALC fits it aggregates.
